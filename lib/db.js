...
export async function getVotingStatistics() {
  try {
    console.log("DB - Getting voting statistics");
    
    // Get all assessments - only include complete votes
    const allAssessments = await getAllAssessments();
    const completeAssessments = allAssessments.filter(a => a && a.is_complete === true);
    console.log(`DB - Retrieved ${allAssessments.length} assessments, ${completeAssessments.length} complete assessments`);

    if (!completeAssessments || completeAssessments.length === 0) {
      console.log("DB - No complete assessments found");
      return {
        totalVoters: 0,
        completeVotes: 0,
        candidateStats: [],
    ...
    // Count unique voters with complete assessments
    const uniqueVoters = new Set(completeAssessments.filter(a => a && a.voter_id).map((assessment) => assessment.voter_id));
    const totalVoters = uniqueVoters.size;
    console.log(`DB - Found ${totalVoters} unique voters with complete assessments`);

    // Group assessments by candidate for statistics
    const candidateStats = {};

    completeAssessments.forEach((assessment) => {
      if (!assessment || !assessment.candidate_id) {
        console.log("DB - Skipping invalid assessment", assessment);
        return;
      }
    ...
    return {
      totalVoters,
      completeVotes: totalVoters, // Semua voter yang dihitung sudah pasti memiliki voting lengkap
...
// Fungsi untuk mendapatkan semua responden dengan informasi waktu pengisian
export async function getAllRespondents() {
  try {
    console.log("DB - Getting all respondents");
    
    // Mendapatkan semua responden dengan informasi waktu pengisian dan status voting
    const result = await sql`
      SELECT 
        v.id, 
        v.email, 
        ...
        v.has_voted,
        CAST(COUNT(DISTINCT ia.candidate_id) AS INTEGER) as assessed_candidates,
        CAST(COUNT(DISTINCT CASE WHEN ia.is_complete = true THEN ia.candidate_id ELSE NULL END) AS INTEGER) as complete_assessments,
        MAX(ia.updated_at) as last_assessment_time
      FROM 
        voters v
      LEFT JOIN 
        integrated_assessments ia ON v.id = ia.voter_id
      GROUP BY 
        v.id, v.email, v.created_at, v.updated_at, v.has_voted
      ORDER BY 
        last_assessment_time DESC NULLS LAST
    `;
    
    console.log(`DB - Retrieved ${result.length} respondents`);
    return result;
  } catch (error) {
